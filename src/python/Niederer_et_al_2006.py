...
def monitor_indices(*monitored):
    ...
    monitor_inds = dict([("overlap", 0), ("K_2", 1), ("K_1", 2), ("Ca_50",\
        ...
        12), ("Ca_i", 13), ("Ca_b", 14), ("ExtensionRatio", 15),\
        ("lambda_prev", 16), ("dExtensionRatiodt", 17), ("lambda_", 18),\
        ("T_0", 19), ("dz_dt", 20), ("dQ_1_dt", 21), ("dQ_2_dt", 22),\
        ("dQ_3_dt", 23), ("dTRPN_dt", 24)])
...
def rhs(states, t, parameters, values=None):
    ...
    beta_0, Ca_50ref, K_z, alpha_0, alpha_r1, alpha_r2, beta_1, n_Hill,\
        ...
        Ca_TRPN_Max, gamma_trpn, k_Ref_off, k_on = parameters
    ...
    # Expressions for the Myofilaments component
    ExtensionRatio = 1
    dExtensionRatiodt = 0
    lambda_ = (ExtensionRatio if ExtensionRatio > 0.8 and ExtensionRatio <=\
        1.15 else (1.15 if ExtensionRatio > 1.15 else 0.8))
...
def monitor(states, t, parameters, monitored=None):
    ...
    monitored[17] = 0
    monitored[18] = (monitored[15] if monitored[15] > 0.8 and monitored[15]\
        <= 1.15 else (1.15 if monitored[15] > 1.15 else 0.8))

    # Expressions for the Filament overlap component
    monitored[0] = 1 + beta_0*(-1 + monitored[18])
    ...
    monitored[2] = alpha_r2*n_Rel*math.pow(K_z, n_Rel)*math.pow(z_p, -1 +\
        n_Rel)/((math.pow(K_z, n_Rel) + math.pow(z_p, n_Rel))*(math.pow(K_z,\
        n_Rel) + math.pow(z_p, n_Rel)))
    monitored[3] = Ca_50ref*(1 + beta_1*(-1 + monitored[18]))
    monitored[4] = Ca_TRPN_Max*monitored[3]/(k_Ref_off*(1 - (0.5 +\
        0.5*beta_0*(-1 + monitored[18]))/gamma_trpn)/k_on + monitored[3])
    ...
    monitored[6] = alpha_r1 + alpha_r2*math.pow(z, -1 + n_Rel)/(math.pow(K_z,\
        n_Rel) + math.pow(z, n_Rel))
    monitored[20] = (1 - z)*monitored[5] - monitored[6]*z
    ...
    # Expressions for the Isometric tension component
    monitored[19] = monitored[0]*monitored[8]

    # Expressions for the Cross Bridges component
    monitored[9] = Q_1 + Q_2 + Q_3
    monitored[10] = ((1 + a*monitored[9])*monitored[19]/(1 - monitored[9]) if\
        monitored[9] < 0 else (1 + (2 + a)*monitored[9])*monitored[19]/(1 +\
        monitored[9]))
    monitored[21] = A_1*monitored[17] - alpha_1*Q_1
    monitored[22] = A_2*monitored[17] - alpha_2*Q_2
    monitored[23] = A_3*monitored[17] - alpha_3*Q_3
